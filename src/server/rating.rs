...
/// What holding `ours` against `theirs` scored: a win, a draw, or a loss.
///
...
fn outcome(ours: usize, theirs: usize) -> f64 {
...
mod tests {
    ...
    fn a_rating_converges_on_who_keeps_winning() {
        ...
        for game in 0..300 {
            ...
            let (ours, theirs) = if game % 3 == 2 { (4, 9) } else { (9, 4) };
            let after = after(&[
                Entrant { rating: me, team: 1, score: ours, games: PROVISIONAL_AFTER },
