...
fn seat_json(server: &crate::server::Server, seat: PlayerId) -> Value {
    ...
    json!({
        ...
        // driving it, which this server knows nothing about. A searching seat
        // has one: the level is what its budget of rollouts comes from.
        "level": bot.filter(|b| !matches!(b.driver, Driver::External)).map(|b| b.level),
