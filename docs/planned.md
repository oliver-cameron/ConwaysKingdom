...
`examples/two` runs beside a hard bot and the peers agree with each other: three or four generations in four hundred differ, each put right within one, which is alice's own prediction arriving. What the run also shows is the server correcting both peers at every checkpoint, on the bot's chunk last, where without a bot it corrects them at the first three and then stops. **That is not the bot's doing**: `a_peer_built_from_steps_agrees_with_the_server_with_a_bot_in_the_room` starts a peer from the server's world, feeds it nothing but `Step`s and finds it the server's world for two hundred generations, chunk for chunk. So what the example disagrees with the server about is on its fetch side, and a bot only makes it show more often by keeping a chunk moving; why a chunk fetched whole at one checkpoint is wrong again by the next is not settled here, and the real client fetches the same way.
