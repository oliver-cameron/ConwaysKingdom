...
A mine that dies now costs what a birth pays, so income is the net change in your mine population and a pattern that merely churns earns nothing. That is what stops mining being the answer to everything: three mines beside a starting block used to convert the whole colony and earn about eight a generation forever, which recovered the starting purse in seconds.

What is left is the numbers. `MINE_COST`, `MINE_YIELD` and `MINE_DRAIN` are one decision between them — ten, one and one means a mine has to grow into ten more before it has paid for itself, which may now be too *harsh* rather than too generous, since only genuinely growing patterns earn at all. Setting the drain below the yield brings churn back. It wants play rather than argument.
...
- **Territory decays now**, one generation in sixteen where nothing alive is touching it, with granted ground exempt as the floor. What is unsettled is the rate and the floor: sixteen is a guess, and "your home patch is permanent" is a strong promise that also means an opponent who grows over it keeps a square that will never decay for them either.
