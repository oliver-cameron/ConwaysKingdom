...
**A new world is made from the same screen**, under the list and above "play alone". One press opens the form where the button was rather than on a screen of its own, and it asks one thing per row: a name, a shape, a size if the shape wraps, whether it ends, and the target if it does. A row appears only when the decision it belongs to is live — there is no size to give a boundless world and no target for one that never ends — which is borrowed from [Infinite Chess](https://www.infinitechess.org/), where the difficulty appears only once the opponent is a computer.
