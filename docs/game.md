...
Every dead cell carries an owner **and a level** — how much of that owner's influence reaches it, nought to seven — and that is territory. It is drawn: ground fades from full colour to almost nothing as a claim thins out, so the map shows where the pressure is rather than only where the border ended up. The rule spreads it: a dead cell next to living ones takes one of their owners most generations, so ground is claimed by the life that grows over it. It stays dead — this sets the owner and nothing else. Ice is exempt while it stands, since a pane's cover is not claimed out from under it.

**Placing is priced by how far your influence reaches**, and refused where none of it does. Ownership is a level now rather than a flag, so there is something to grade it by: at full strength a placement costs its own price, and every level thinner adds a multiple, so the edge of your reach costs seven times what the middle does.

Ten times flat, everywhere outside, was tried and was too weak — and it was flat because a flag has no gradient. A wall was tried before that and was worse: a player whose life went out was finished, since they could not place, so could not grow ground, so could not place. The wall is back with a slope in front of it, and it is safe this time for a reason that is nothing to do with the price: **granted ground is a source**, so everybody always has a patch with a live gradient to build on.

The price is per **cell**, not per action, because a stroke that starts in the middle of your ground and runs off the edge is the ordinary case; charging the whole of it at one rate would be wrong in one direction or the other. `net::cell_cost` is the whole rule, and the client prices on the same terms the server does.
