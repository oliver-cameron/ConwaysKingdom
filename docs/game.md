...
**Going back gives the seat up**, with `ClientMessage::Leave`. It used to send nothing, on the reasoning that the seat is held until another join takes its place — which is true of somebody rejoining the same room and false of everything else: the player stayed marked online, so the room went on counting them, and the rejoin token, which only returns you to a player who is **not** online, found them online and issued a new player instead. Leave and come back three times and a room with one person in it said three. The token is kept, so coming back is still coming back to the same player, the same value and the same ground.
