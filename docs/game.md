...
**A bot balances the sides.** Each team in the lobby has an "Add a bot" beside it — one for the room, in a free-for-all — and a picker for how hard it plays: easy earns, normal earns and contests the frontier, hard also walls what it built. Anybody seated may add one and anybody seated may remove one, which is the "remove" beside a name marked "(bot)"; a spectator may do neither. It takes one of the fifteen seats, so the count on the panel counts it, and gives it back if it is removed before the whistle; once the whistle has gone it is settled like a side — a seat leaving a running match is a forfeit. What it plays is [the server's](server.md#bots).
...
**Escape leaves whatever you are in**, one level at a time and innermost first: a text field lets go of the keyboard **and of its selection**, then the page goes back, then a half-drawn rectangle is abandoned. Surrendering focus alone left the highlight painted where it was, so the field looked as though it still had the keyboard and there was nothing left to press. The innermost rung is the one that was missing and the one that was most annoying — egui takes the keyboard while a field has focus, so escape never reached the game, and a highlighted selection in a field you could not leave had no way out at all. Every screen having a key out of it is the habit taken from [chess-tui](https://github.com/thomas-mauran/chess-tui), and the key leaves every page rather than only the room list: a page goes back to wherever its own back button goes, because `Page::back` is the one answer both ask. The two pages under the account page — people, and parties — go back to it rather than past it to home, which is what a page reached from a button two steps in has to do for the button and the key to agree. There is a way back by pointer too — an arrow in the corner of the HUD, and a button in the lobby, return to the menu from wherever you are. The socket is kept and the room list asked for again, so going back is a step rather than a disconnection.
