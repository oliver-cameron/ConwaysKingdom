...
**There is no button for asking a server what it has.** That is what the address is for, and a field followed by a button meaning only "yes, that address" is one control too many. The client reaches when the typing settles: on enter, on leaving the field, or after a pause, whichever comes first. Debounced rather than fired per keystroke, because `ws://127.0.0.1:8080/ws` passes through twenty addresses on its way to being one and every one of them would open a socket — and asked once per address, or the pause would fire on every frame after it. A failure is **asked again on its own**, every few seconds, because the usual reason is a server that is not running *yet* — somebody is starting it in the other window — and a menu that gives up after one refusal makes that a thing you have to notice and click. The button beside the refusal is for whoever does not want to wait out the interval.
