...
`views::social` is the exception, and it is grouped by **subject** rather than by screen: who you are, who else plays here, what a server will say about any of them, the parties you are in, the doors held open for you, and the face that tells two players with one name apart. Those were spread across the other two — `account` and `people` were menu pages, `profile` is a panel over a running board — and they are one subject, filed against a `net::PersonId` rather than against a seat, because a person outlives every world on a server. Anything drawn there that another player sees has to be the server's; the exception is what a server merely *holds* and shows nobody, which is `net::kept`.
...
It is a **struct rather than a tree of constants**, and that is the difference between one language and several. `words::Words` names every string as a field, each language is one `static` filling it in — `words::eng` is the first — and `words::w()` hands back whichever is current. Nothing is copied: a language is built at compile time out of string literals, so reading one is a pointer and choosing one is a pointer swap. A language that forgot a word does not compile, which is the property a file of constants cannot have.

The fifty-three **formatters** — plurals, counts, "3 of 7 matches won" — are the part this does not yet solve. They live in `words::eng` and are re-exported, so a call site reads as it always did, and that is honest for one language and is not a translation: word order, plural rules and grammatical number all differ. The shape that fits is a trait with a method per formatter, and the day there is a second language that is the change to make.
...
`server::profiles` is what a server can vouch for about each person it has met: the name last joined under, the rating, the matches settled and the most ground held. It is the line player profiles draw — **anything another player is shown has to be the server's**, because client state is self-asserted and a rating you keep is a rating you can type. `client::record` stays as a client's own diary and is never shown to anybody else.
