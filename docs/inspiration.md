...
| [Infinite Chess](https://www.infinitechess.org/) | A game you make joins the same list everybody else reads — there is no "your games" pane | `ClientMessage::Rooms` is the one listing everybody reads, and a room you make goes into it. A party's worlds are the one exception, in `Parties`: they are private to a group, so they could not be in the list everybody reads, and the listing is the group's rather than yours — see [planned.md](planned.md#parties) |
