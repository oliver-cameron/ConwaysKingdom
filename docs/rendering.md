...
Sprites carry **no hue**. A texel is saturation, lightness and coverage; the colour arrives at draw time from the cell's player, so one set of art serves every player.
...
**A player's colour is a row of a fixed table**, `client::views::hue::PALETTE`: fifteen OKLCH triples — lightness, chroma, hue — that the client hands to the shader in the camera uniform, a column each, and reads itself for the swatch beside a name. It used to be a formula, hue stepped by the golden ratio with saturation and lightness cycling on three and five, and it was two formulas: the shader had drifted to a chroma of 0.30 with a linear taper while the swatch used 0.13 with a squared one, so the lobby's colour was a duller version of the board's. One table, two readers, and `a_swatch_is_the_tables_colour` pins that they agree.

The fifteen were chosen at once rather than one at a time: gradient descent on a repulsion energy in OKLab, every colour projected back inside sRGB after each step, over three ranges of lightness. Letting five of the fifteen go dark — a floor of 0.45 rather than 0.6 — was worth the most, because darkness is a third axis the bright band cannot use. The smallest gap is **0.169 in OKLab, against 0.045 for the formula as the shader drew it**, and `the_closest_two_players_are_far_enough_apart` holds a floor of 0.16 under it. `tools/typefaces.html` shows the fifteen with their closest pairs.

A row is what a **full-saturation texel at lightness 0.62** draws as — `hue::L_SWATCH`, which is also what the swatch is drawn at, so the swatch is the board's colour exactly. The rest of the sheet is placed around it. The texel's saturation scales the row's chroma. Its lightness is remapped, not scaled: linear from black to 0.62 and linear again from 0.62 to white, so the reference lands on the row's lightness and white stays white. A plain multiplier could not do both — the palest rows want 0.8 / 0.62, and the sheet's brightest live texel is at 0.84, which that carries past white, where no chroma survives. An offset of +0.18 does the same at the top and at the bottom would crush the dark rows' dead-ground tiles into black. Below the reference the map *is* a multiplier, which is where the art's shading lives.

**Held ground has a floor that live cells do not.** Five of the fifteen rows sit at a lightness of 0.45, which below the reference is a multiplier of 0.73: above the two thirds under which a cell stops reading as its own art, and right for a live cell, where the separation the table was optimised for is needed. It is wrong for the dead tile. Its body sits at 0.235 on the sheet and the backdrop's at 0.302, so territory is already darker than the ground it has to stand out from and is told from it by hue and chroma rather than by shading — at 0.73 it lands at 0.17 and a dark player's ground sinks into the backdrop, which is what the old tiers did to player 2 at 0.83. So `player_lightness` takes the tile's alive bit: a live cell takes the row in full, and a dead one has its reference floored at `hue::HELD_FLOOR` — 0.85 — of the swatch lightness, which floors the multiplier at 0.85 and leaves hue and chroma alone. The player's live cells still carry the whole of the darkness. The coarse level of detail needs nothing of its own, because `CoarseTexture` uploads the cell's bytes and the shader colours them through the same function; the one CPU path that draws the sheet, `views::icons::tint`, reads a texel's state off its column so the hotbar's picture of held ground is the board's.
